...
```text
https://cdn.jsdelivr.net/npm/@kubohiroya/turbowarp-tmpose@1.10.2/dist/tmpose.js
```
...
```text
https://cdn.jsdelivr.net/npm/@kubohiroya/turbowarp-tmpose@1.10.2/dist/runtime.js
```

To add the published package to another project:

```sh
pnpm add --save-exact @kubohiroya/turbowarp-tmpose@1.10.2
```
...
TMPose owns the camera canvas used by Teachable Machine Pose and TensorFlow.js. Before the first
frame, it checks the active TensorFlow backend. The CPU backend receives the Canvas2D
`willReadFrequently` hint because its `fromPixels()` path reads pixels through Canvas2D; WebGL and
unknown backends receive a normal 2D context so accelerated video drawing is preserved. Preview
rendering and model input continue to use the same canvas, and no downstream Canvas prototype
patch is required. Roll back by pinning the preceding TMPose version; do not replace this boundary
with global Canvas or Console interception.
